var RepairMedia = function() {
};
  
RepairMedia.prototype.run = function(future, subscription) {  
    var args = this.controller.args;

    console.log("Tailor/RepairMedia: Called by "+this.controller.message.applicationID().split(" ")[0]+
		" via "+this.controller.message.senderServiceName());

    var argv = ["/usr/sbin/fsck.vfat", "-y", "-v", "-V", "/dev/store/media"];

    console.log("Tailor/RepairMedia: Running command: "+argv.join(' '));

    var command = spawn(argv[0], argv.slice(1));

    console.log("Tailor/RepairMedia: Spawned child (pid "+command.pid+")");
